...
describe('server collection helpers', () => {
	...
	it('queries and upserts teams', async () => {
		...
		const teams = setCollection('teams', [
			{ id: 'team-1', name: 'McLaren', year: new Date().getFullYear() }
		]);
	...
	it('filters next-race and user predictions', async () => {
		const { getNextRacePredictionsQuery, getPredictionsQuery, getUserPredictionsQuery } =
			await import('./predictions');
		setCollection('races', [
			{
				id: 'race-1',
				year: new Date().getFullYear(),
				sessions: [{ date: '20 Jun', time: '12:00' }]
			}
		]);
		...
		expect(predictions.getFullList).toHaveBeenCalledWith({
			expand: 'user,race',
			filter: `race='race-1'`
		});
		expect(predictions.getFullList).toHaveBeenCalledWith({
